...
export class BambuleScraper extends BaseScraper {
  constructor(
        ...
        for (const item of items) {
          try {
            ...
            const priceText = $item
              .find('[class*="price"]:not([class*="old"])')
              .first()
              .text()
              .trim()
  ...
  private parseStockStatus(text: string): StockStatus {
    ...
    if (lower.includes('skladem') || lower.includes('dostupn') || lower.includes('online')) {
      return 'in_stock'
    }
    if (lower.includes('vyprodáno') || lower.includes('nedostupn')) {
      return 'out_of_stock'
    }
    if (lower.includes('předobjednávka') || lower.includes('pripravujeme')) {
      return 'preorder'
    }
    return 'unknown'
  }
