...
export class SparkysScraper extends BaseScraper {
  constructor(
  ...
  private parseStockStatus(text: string): StockStatus {
    ...
    if (lower.includes('skladem') || lower.includes('dostupn')) return 'in_stock'
    if (lower.includes('vyprodáno') || lower.includes('nedostupn') || lower.includes('není skladem')) return 'out_of_stock'
    if (lower.includes('předobjednávka') || lower.includes('připravujeme')) return 'preorder'
    return 'unknown'
  }
}
