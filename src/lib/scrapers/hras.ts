...
export class HrasScraper extends BaseScraper {
  constructor(
  ...
  private parseStockStatus(text: string): { status: StockStatus; quantity: number | null } {
    ...
    if (lower.includes('in stock') || lower.includes('skladem') || lower.includes('skladom')) {
      const qtyMatch = text.match(/\(>?(\d+)\s*(?:ks|pcs)\)/i)
      return { status: 'in_stock', quantity: qtyMatch ? parseInt(qtyMatch[1], 10) : null }
    }
    if (lower.includes('on request') || lower.includes('na dotaz')) {
      return { status: 'out_of_stock', quantity: null }
    }
    if (lower.includes('vyprodáno') || lower.includes('out of stock')) {
      return { status: 'out_of_stock', quantity: null }
    }
