...
export class PompoScraper extends BaseScraper {
  constructor(
  ...
  async fetchProducts(): Promise<RawProduct[]> {
    ...
    while (page <= maxPages) {
      ...
      try {
        ...
        const items = $(
          '.product-list .product, .product-item, [class*="product-card"], [data-product]'
        ).toArray()
  ...
  private parseStockStatus(text: string): {
    ...
    if (lower.includes('skladom') || lower.includes('skladem') || lower.includes('online')) {
      const qtyMatch = text.match(/(\d+)\s*ks/i)
      return {
        status: 'in_stock',
    ...
    if (lower.includes('nedostupn') || lower.includes('vypredané') || lower.includes('vypredane') || lower.includes('vyprodáno')) {
      return { status: 'out_of_stock', quantity: null }
    }

    if (lower.includes('predobjednávka') || lower.includes('předobjednávka') || lower.includes('pripravujeme')) {
