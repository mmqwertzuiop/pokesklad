...
export class ShadowballScraper extends BaseScraper {
  constructor(
  ...
  private parseStockStatus(text: string): { status: StockStatus; quantity: number | null } {
    ...
    if (lower.includes('vyprodáno') || lower.includes('vyprodano') || lower.includes('sold out')) {
      return { status: 'out_of_stock', quantity: null }
    }
    if (lower.includes('předobjednávka') || lower.includes('preorder')) {
