...
export class CardempireScraper extends BaseScraper {
  constructor(
  ...
  private parseStockStatus(text: string): { status: StockStatus; quantity: number | null } {
    ...
    if (lower.includes('in stock') || lower.includes('skladom') || lower.includes('skladem')) {
      const qtyMatch = text.match(/\(>?(\d+)\s*(?:ks|pcs)\)/i)
      return { status: 'in_stock', quantity: qtyMatch ? parseInt(qtyMatch[1], 10) : null }
    }
    if (lower.includes('ordered') || lower.includes('objednané') || lower.includes('preorder')) {
      return { status: 'preorder', quantity: null }
    }
    if (lower.includes('ask us') || lower.includes('vypredané') || lower.includes('vyprodáno') || lower.includes('nedostupn')) {
